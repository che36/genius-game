...
const changeColor = (computer=false,button) => {
	if(action)
		return
	if(!computer && !turn)
		return gameOver('Espere pelo sinal do Go!.')
	if(!computer && turn && performance.now() - go > maxTimeAllowed)	
		return gameOver('Você demorou demais.')
	if(!computer && turn && buttons.indexOf(button) != pattern[i++])
		return gameOver('Escolheu a sequencia errada.')
	action = true
	go = performance.now()
	...
	setTimeout( () => {
		...
		if(!computer && turn && i == pattern.length){
			setTimeout( () => {
				...
				radial.c.fillText("Ótimo", radial.w2-42, radial.h2+10)
				if(sound){
